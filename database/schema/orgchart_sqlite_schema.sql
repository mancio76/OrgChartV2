--
-- File generated with SQLiteStudio v3.4.17 on dom lug 27 10:04:28 2025
--
...
(
  ...
  percentage       REAL     NOT NULL
                            DEFAULT 1.0
                            CHECK (percentage > 0 AND
                                   percentage <= 1),
  is_ad_interim    BOOLEAN  NOT NULL
                            DEFAULT (0),
  is_unit_boss     BOOLEAN  NOT NULL
                            DEFAULT (0),
  notes            TEXT,
  flags            TEXT,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_assignments_version ON person_job_assignments
(
  person_id,
  unit_id,
  job_title_id,
...
CREATE VIEW IF NOT EXISTS get_complete_tree AS
...
CREATE VIEW IF NOT EXISTS unit_get_list_query AS
  SELECT u.*,
  ...
  FROM units u
       LEFT JOIN
       units p ON u.parent_unit_id = p.id
       LEFT JOIN
       units c ON c.parent_unit_id = u.id
       LEFT JOIN
       current_assignments ca ON ca.unit_id = u.id
  GROUP BY u.id,
           u.name,
           u.short_name,
           u.unit_type_id,
...
CREATE VIEW IF NOT EXISTS units_hierarchy AS
  WITH RECURSIVE unit_tree AS
  (-- Nodi radice (parent_unit_id = -1 o NULL)
    SELECT id,
           unit_name name,
  ...
  SELECT ut.*,
         ca.person_id AS boss_person_id,
         ca.job_title_id AS boss_job_title_id
  FROM unit_tree ut
       LEFT OUTER JOIN
       current_assignments ca ON ut.id = ca.unit_id AND
                                 ca.is_unit_boss
  ORDER BY ut.path;


-- View: units_hierarchy_stats
CREATE VIEW IF NOT EXISTS units_hierarchy_stats AS
  SELECT uh.id,
         uh.name,
         uh.short_name,
         uh.unit_type,
  ...
  FROM units_hierarchy uh
       LEFT OUTER JOIN
       person_job_assignments pja ON uh.id = pja.unit_id
  GROUP BY uh.id,
           uh.name,
           uh.short_name,
           uh.unit_type,
...
CREATE VIEW IF NOT EXISTS units_types AS
  ...
  FROM units u
       JOIN
       unit_types ut ON u.unit_type_id = ut.id;
...
CREATE TRIGGER IF NOT EXISTS update_assignable_units_timestamp
AFTER UPDATE
ON job_title_assignable_units
WHEN NEw.datetime_updated <> OLD.datetime_updated
BEGIN
  UPDATE job_title_assignable_units
...
CREATE TRIGGER IF NOT EXISTS update_job_titles_timestamp
AFTER UPDATE
ON job_titles
WHEN NEw.datetime_updated <> OLD.datetime_updated
BEGIN
...
-- Trigger: update_unit_types_timestamp
CREATE TRIGGER IF NOT EXISTS update_unit_types_timestamp
AFTER UPDATE
ON unit_types
WHEN NEW.datetime_updated <> OLD.datetime_updated
BEGIN
