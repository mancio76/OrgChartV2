...
INSERT OR IGNORE INTO unit_types (id, name, aliases) VALUES 
(1, 'Function', '[]'),
...
CREATE VIEW IF NOT EXISTS units_hierarchy AS WITH RECURSIVE unit_tree AS (-- Nodi radice (parent_unit_id = -1 o NULL)
    ...
    SELECT u.id,
           ...
           u.parent_unit_id,
           tree.level + 1,
           tree.path || '/' || CAST (u.id AS TEXT),
           tree.full_path || ' > ' || u.name
      FROM units u
           JOIN unit_types ut ON u.unit_type_id = ut.id
           JOIN unit_tree tree ON u.parent_unit_id = tree.id
)
SELECT *
  FROM unit_tree
 ORDER BY path;
...
CREATE TRIGGER IF NOT EXISTS update_unit_types_timestamp AFTER UPDATE ON unit_types WHEN NEw.datetime_updated <> OLD.datetime_updated BEGIN UPDATE unit_types SET datetime_updated = CURRENT_TIMESTAMP WHERE id = NEW.id; END;
