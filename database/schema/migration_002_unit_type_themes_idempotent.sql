-- Migration 002: Unit Type Themes System (Idempotent Version)
...
-- ============================================================================
-- UNIT_TYPES TABLE ENHANCEMENT (Conditional)
-- ============================================================================

-- Create index for the theme_id foreign key (safe to run multiple times)
CREATE INDEX IF NOT EXISTS idx_unit_types_theme_id ON unit_types(theme_id);

-- ============================================================================
-- DEFAULT THEME DATA INSERTION (Idempotent)
-- ============================================================================

-- Insert default themes matching current hardcoded styling (only if they don't exist)
-- Theme 1: Function Theme (for unit_type_id = 1)
INSERT OR IGNORE INTO unit_type_themes (
    id,
    name, 
    description, 
    icon_class, 
...
-- ============================================================================
-- ASSIGN THEMES TO EXISTING UNIT TYPES (Idempotent)
-- ============================================================================

-- Assign Function Theme to unit_type_id = 1 (Function) - only if not already assigned
UPDATE unit_types 
SET theme_id = 1
WHERE id = 1 AND (theme_id IS NULL OR theme_id != 1);

-- Assign Organizational Theme to unit_type_id = 2 (OrganizationalUnit) - only if not already assigned
UPDATE unit_types 
SET theme_id = 2
WHERE id = 2 AND (theme_id IS NULL OR theme_id != 2);
